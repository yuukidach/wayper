...
- **智能标签排除** — 分析拉黑历史，自动发现反复拉黑的标签。支持单标签和组合排除（如"tattoo + nude"），精细过滤。已排除的标签会自动抑制相关建议。
- **跨平台 GUI** — 浏览、预览、搜索和管理整个壁纸集合。标签搜索、智能建议、daemon 控制、设置——全键盘操作。
...
- **设置** — 在 GUI 中配置 Wallhaven 查询、排除标签/组合、纯度和显示器
